...
/**
 * Debugging tool to output packet_t fields
 */
void dump_packet(packet_t *p) {
    printf("PACKET INFO\n");
    printf("mtype: %s\n", (p->mtype == ACK ? "ACK" : "DATA"));
    printf("message_id: %d\n", p->message_id);
    printf("pid: %d\n", p->pid);
...
void fresh() {
    total_packets = 0;
    next_packet = 0;
    num_available_packets; // number of packets that can be sent (0 <= n <= WINDOW_SIZE)
    is_receiving = 0; // a helper varibale may be used to handle multiple senders
    waiting_on_first = 0;
    time_outs = 0;
    window_num = 0;
    received_packets = 0;
...
int init(char *process_name, key_t key, int wsize, int delay, int to, int drop) {
    ...
    if(mailbox_id == -1) {
        perror("init: msgget failed");
        return -1;
    }

    // TODO set the signal handler for receiving packets
    struct sigaction action_io;
    action_io.sa_handler = receive_packet;
    sigfillset(&action_io.sa_mask);
    action_io.sa_flags = 0;
    sigaction(SIGIO, &action_io, NULL);
    fcntl(myinfo.pid,F_SETFL,O_ASYNC);
    //Alarm handler for timeouts
    struct sigaction action_alrm;
    action_alrm.sa_handler = timeout_handler;
    action_alrm.sa_flags = 0;
    sigfillset(&action_alrm.sa_mask);
    sigaddset(&action_alrm.sa_mask,SIGIO);   //block SIGIO while in alarm handler
...
int send_packet(packet_t *packet, int mailbox_id, int pid) {
    ...
    if(msgsnd(mailbox_id, packet, sizeof(packet_t), 0) == -1) {
        perror("send_packet: msgsnd failed");
        return -1;
    }
    if(kill(pid, SIGIO) == -1) {
        perror("send_packet: SIGIO failed");
        return -1;
    }
    return 0;
}
...
int send_message(char *receiver, char* content) {
    ...
    if (create_packets(content, &message_stats) < 0) {
        ...
        free((void *)message_stats.packet_status);
        return -1;
    }
    ...
    // Probe for a message id by sending one message and waiting for the ACK
    int num = get_next_packet(message_stats.num_packets);
    waiting_on_first = 1;
    while (waiting_on_first) {
        send_packet(&message_stats.packet_status[num].packet, message_stats.mailbox_id, message_stats.receiver_info.pid);
        message_stats.packet_status[num].is_sent = 1;
        printf("Sent packet #%d to pid: %d\n",num,message_stats.receiver_info.pid);
        if(time_outs >= MAX_TIMEOUT){
            return 0;                                                // if max timeouts reached--return
        }
        alarm(TIMEOUT);
        pause();
        sleep(1);                            // allow time to receive ACK
    }
    if(message_stats.num_packets > 1) {
        int i;
        num = get_next_packet(message_stats.num_packets);
        for(i = 0; i < WINDOW_SIZE && i + 1 < message_stats.num_packets && num >= 0; i++) {
            send_packet(&message_stats.packet_status[num].packet, message_stats.mailbox_id, message_stats.receiver_info.pid);
            message_stats.packet_status[num].is_sent = 1;
            printf("Sent packet #%d to pid: %d\n",num,message_stats.receiver_info.pid);
            num = get_next_packet(message_stats.num_packets);
        ...
        while(received_packets < message_stats.num_packets && time_outs < 5){
            num = get_next_packet(message_stats.num_packets);
            if(next_packet && num >= 0){
                send_packet(&message_stats.packet_status[num].packet, message_stats.mailbox_id, message_stats.receiver_info.pid);
                message_stats.packet_status[num].is_sent = 1;
                next_packet = 0;
                printf("Sent packet #%d to pid: %d\n",num,message_stats.receiver_info.pid);    
    ...
    alarm(0);
    return (message_stats.num_packets_received == message_stats.num_packets ? 0 : -1);
}
...
void timeout_handler(int sig) {
    int i;
    time_outs++;
    if(time_outs >= MAX_TIMEOUT && timed_out == 0) {
        alarm(0);
				timed_out = 1;
        printf("Sender timed out\n");
    } else if(time_outs >= MAX_TIMEOUT && timed_out == 1){
				alarm(0);
			}	else if(message_stats.num_packets_received < message_stats.num_packets) {
        printf("TIMEOUT\n");
        for (i = 0; i < message_stats.num_packets; i++){
            if (message_stats.packet_status[i].is_sent == 1 && message_stats.packet_status[i].ACK_received == 0){
                ...
                printf("Resent packet #%d to pid: %d\n",i,message_stats.receiver_info.pid);
            }    
        }
        alarm(TIMEOUT);
        sleep(1);                    // allow time for ACK response
    }
}
...
int send_ACK(int mailbox_id, pid_t pid, int packet_num) {
    ...
    ackpack.mtype = ACK;
    ackpack.pid = myinfo.pid;
    ackpack.packet_num = packet_num;
    ackpack.message_id = message_id;
    ackpack.total_size = sizeof(ackpack.data);
    int delay = rand() % MAX_DELAY;
    sleep(delay);

    // TODO send an ACK for the packet it received
    return send_packet(&ackpack, mailbox_id, pid);
}
...
void handle_data(packet_t *packet, process_t *sender, int sender_mailbox_id) {
    //copy packet data into raw_data at the proper index to concat once total message is received
    int data_size = (packet->packet_num + 1 == packet->num_packets ? packet->total_size % PACKET_SIZE : PACKET_SIZE);
    strncpy(raw_data[packet->packet_num].raw, packet->data, data_size);
    // raw_data[packet->packet_num].raw[PACKET_SIZE] = '\0';
    if(send_ACK(sender_mailbox_id, sender->pid, packet->packet_num) == -1) {
        perror("receive_packet: can't send ACK");
    } else {
        is_receiving = 1;
        printf("Sent ACK #%d\n", packet->packet_num);
    }
    total_packets++;
    // if all packets received then end receiver process
    if(total_packets >= message.num_packets){
...
void handle_ACK(packet_t *packet) {
    int num = packet->packet_num;
    message_stats.packet_status[num].ACK_received = 1;
    message_stats.num_packets_received++;
    next_packet = 1;
    received_packets++;
    
    // if first packet set sender message ids to receiver message ids
    if (waiting_on_first){
        int i;
        for(i = 0; i < message_stats.num_packets; i++){
    ...
    // print if hasn't timed out or hasn't received all ACKS
    if(time_outs < MAX_TIMEOUT && received_packets <= message_stats.num_packets){
        time_outs = 0;
...
void receive_packet(int sig) {
    ...
    is_receiving = 0;
    if(msgrcv(mailbox_id, &received_packet, sizeof(packet_t), 0, IPC_NOWAIT) == -1) {
        perror("receive_packet: can't read incoming packet");
    } else {
        // printf("\n");
        // dump_packet(&received_packet);
        // printf("\n");
        process_t sent_by;
        int legit_sender = get_process_info(received_packet.process_name, &sent_by);
        if(legit_sender == 0) {
            if(received_packet.mtype == DATA) {
                if(received_packet.message_id == -1) {
                     //New message coming through!
                    if(is_receiving) {            // But I'm already working on something
                        if(kill(received_packet.pid,SIGALRM) == -1){
                            perror("receiving another message");
                        }
                    } else { // It's fresh
                        if(drop_packet()){
                            if(kill(received_packet.pid,SIGALRM) == -1){
                            ...
                            is_receiving = 1;
                            raw_data = malloc(sizeof(message_t) * received_packet.num_packets);                         
                            message_id++;
                            message.is_complete = 0;
                            message.num_packets = received_packet.num_packets;
                            int sender_id = msgget(sent_by.key, 0666);
                            handle_data(&received_packet,&sent_by,sender_id);
                        }
                    }
                } else {
                    if(drop_packet()){
            ...
            } else if(received_packet.mtype == ACK) {
                if(drop_packet()){
                    raise(SIGALRM);
                } 
                else{                   
                    handle_ACK(&received_packet);
                }
            }
        } else {
            perror("receive_packet: packet from mystery sender");
        }
    }
}
...
int receive_message(char *data) {
    ...
    raw_data = malloc(sizeof(message_t));
    
    // pause until message is complete
    while(!message.is_complete){
        pause();
    }
    sleep(2);
    alarm(0);
    // build message in proper order/release memory/reset globals 
    message.data = raw_data[0].raw;
    for(i = 1; i < message.num_packets;i++){
        strncat(message.data, raw_data[i].raw, strlen(raw_data[i].raw));
    }
    strcpy(data, message.data);                               // save message to data
    int successful = message.is_complete;
    message.is_complete = 0;
    total_packets = 0;    
